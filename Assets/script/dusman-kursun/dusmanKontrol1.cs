...
public class dusmanKontrol1 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            if ((karakter.transform.position.y - transform.position.y) < 30 && (karakter.transform.position.x - transform.position.x) < 30 && (karakter.transform.position.y - transform.position.y) > -30 && (karakter.transform.position.x - transform.position.x) > -30)
            {
...
#if UNITY_EDITOR
[CustomEditor(typeof(dusmanKontrol1))]
[System.Serializable]
class dusmanKontrol1Editor : Editor
{

    public override void OnInspectorGUI()
    {
        dusmanKontrol1 script = (dusmanKontrol1)target; // ust classtaki değişkenlere ulasmamı sağlar
